...
int Goomba::checkCollision(const glm::vec2& posPlayer, bool super, bool star, bool down)
{
	...
	if (abs(posPlayer.x - pos.x) <= 14 and abs(posPlayer.y - pos.y) <= 21 + playerHeight and abs(posPlayer.y - pos.y) >= 13 + playerHeight and down) {
		sprite->changeAnimation(1, 0);
		deathTime = time;
		return 2; //Return 2 si eliminem a l'enemic
	}
	if (abs(posPlayer.x - pos.x) <= 14 and ((!super and abs(posPlayer.y - pos.y) <= 14) or (super and pos.y >= posPlayer.y - 13 and pos.y <= posPlayer.y + 29))) {
		if (star) {
			sprite->changeAnimation(1, 0);
			deathTime = time;
			return 3;
